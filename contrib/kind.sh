...
if [ "$OVN_MONITOR" == true ]; then
  ...
  popd
  echo "Pods are all up, your Prometheus instance is now accessible on localhost:9090"
  # Expose the prometheus service so that we can access it from localhost:9090
  exec kubectl -n monitoring port-forward svc/prometheus-k8s 9090
fi
